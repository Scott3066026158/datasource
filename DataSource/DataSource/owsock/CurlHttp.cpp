...
namespace OwLib
{
	...
	{ 
		...
		{ 
			...
			ret = (int)fwrite(buffer, size, nmemb, out->m_stream); 
		} 
		return ret;
	} 
	...
	void CurlHttp::SetCurlProxy(CURL *curl)  
	{
		...
		if ((int)m_proxyInfo->m_address.length() > 0)
		{
	...
	bool CurlHttp::TestProxy(CurlProxyInfo *proxyInfo, const string& url)
	{
		...
		if ((int)proxyInfo->m_address.length() > 0)
		{
		...
		:m_proxyInfo(0),m_hWnd(callhwnd)
	{
		...
		m_Lock.SetName("CurlHttp_m_Lock");
	}
