...
int replace_rename(const char *src, const char *dst) {
    ...
    int retries = 50;
    while (1) {
        if (MoveFileEx((LPCWSTR)src, (LPCWSTR)dst, MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED | MOVEFILE_WRITE_THROUGH)) {
            return 0;
        } else {
            errno = GetLastError();
            if (errno != 5) break;
